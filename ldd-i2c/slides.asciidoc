...
=== I2C Controller/Adapter
  ...
  * Linux refers the I2C controller as I2C bus adapter, and it is
 ...
 * Modify the LCD code
    link:{include:code/lcd/lcd.full.datauri}["lcd.c",filename="lcd.c"], and
...
=== I2C Dev Interface
  ...
  * In Linux, I2C Bus can be accessed using a dev interface,
    `/dev/i2c-<bus no.>`.
...
* Develop a code which reads the i2c-key device's status and data
  register, then prints the key pressed.
 ...
 * Modify the Key code
    link:{include:code/ikey/ikey.full.datauri}["key.c",filename="key.c"], and
