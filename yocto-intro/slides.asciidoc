 ...
 * The tmp/work is the folder where all the packages are unpacked and
   built.

 * for each package we would find a folder for target architecture
   armv5e-oe-linux-gnueabi/${PN}/${PV}-${PR}/

 * The source is unpacked patched in ${PN}-${PV} and compiled in
   _build_ folder
