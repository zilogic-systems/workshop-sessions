...
* Designed to be low in power consumption so that the device with
  add-on boards can be powered from USB.
...
* Try to check the CPU details by printing `/proc/cpuinfo`
