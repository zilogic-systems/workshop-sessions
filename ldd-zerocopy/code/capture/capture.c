...
static void imageProcess(const void* p, int frame_number)
{
	unsigned char* src = (unsigned char*)p;
	unsigned char* dst = malloc(width*height*3*sizeof(char));
	char jpegFilename[32];

	YUV420toYUV444(width, height, src, dst);
	sprintf(jpegFilename, "frame-%d.jpeg", frame_number);
...
static void init_device(void)
{
	...
	fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUV420; //replace
