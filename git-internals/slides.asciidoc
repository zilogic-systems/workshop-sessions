...
Which of the following are True?
  ...
  * [A] Each object in the object database is identified by SHA1 of
    its contents.
  * [B] Commit objects have a reference to their previous commit
  * [C] Commit objects have a reference to a tree object,
    corresponding to the top most directory in the project.
  * [D] Tree object have references to files / directories present
    with the the corresponding directory

=== Quiz

Which of the following are True?

  * [A] If the contents of the file are the same, it is stored only
    once in the object database.

  * [B] The probability of the two different files having the same
    SHA1 is very very low.

  * [C] The HEAD file points to the latest commit.

  * [D] On commit, the branch file is updated to point to newly
    created commit.
