 ...
 * A method is suspended in-between and 
 ...
 * methods of program executed in any order, doesn't affect the final
   result, then they can support concurrency.
...
* UP - Uni Processor
  - only single core, scope for concurrency is just preemption and
    interrupts
...
=== Solving Concurrency Issues
  ...
  * May need to disable pre-emption
...
* Short term locks
  ...
  - can be a busyloop lock, since lock-in period is less.
...
=== Spin Locks
  ...
  * When lock is available, disables process pre-emption in single
    core processor, and sets a lock flag.
